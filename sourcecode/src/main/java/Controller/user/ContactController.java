...
public class ContactController extends HttpServlet {
    ...
    throws ServletException, IOException {
        String btn = request.getParameter("btn-send-message");
        if(btn != null) {
            ...
            while(!isSend && index < 5) {
                isSend = e.sendEmail(emailAdmin,"Contact user", messageContact, null);
                index++;
            }
            if(isSend)  {
                response.sendRedirect("/SWP391-MomAndBaby/contact?act=send-contact&status=1");
            } else {
                response.sendRedirect("/SWP391-MomAndBaby/contact?act=send-contact&status=0");
            }
        } else {
